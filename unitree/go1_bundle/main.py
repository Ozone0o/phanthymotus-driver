            ...
            try:
                if method == "initialize":
                    ok({"protocolVersion": "2024-11-05", "capabilities": {"tools": {}},
                        "serverInfo": {"name": "go1-bundle-zjh", "version": "1.0.0"}})
...
def main():
    ...
    control_level = str(cfg.get("control_level", "HIGHLEVEL")).upper()

    print(f"[bundle] namespace={namespace} mcp_port={mcp_port} control_level={control_level}")

    # 高层运控(HighCmd,.161:8082) 与 低层关节控制(LowCmd+Safety,.10:8007) 互斥，
    # 同一 bundle 运行时按 control_level 只实例化其中一套 client（见 CONTRIBUTING §4）。
    if control_level == "LOWLEVEL":
        from go1_low_sdk_client import Go1LowSdkClient
        client = Go1LowSdkClient(network_iface=network_iface)
    else:
